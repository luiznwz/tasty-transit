...
export default class CartDrawer extends HTMLElement {
  ...
  constructor() {
    ...
    this.cartRelatedProducts = this.querySelectorAll(
      ".cart_related_product__snippet"
    );
    ...
    this.freeShippingPrice = Number(
      this.getAttribute("data-free-shipping-price")
    );
    this.cartShippingPrice = this.querySelector(".cart_shipping_price");
    this.firstProduct = "";

    this.installmentsContent = document.querySelectorAll(
      "[data-product-installments]"
    );
    ...
    if (
      ...
      window.location.search !== "?view=cart-drawer"
    )
      return;
  ...
  private loadRelatedProducts() {
    this.firstProduct = this.querySelector(
      "[data-related-product]"
    ) as HTMLElement;
    const productId = this.firstProduct?.dataset.relatedProduct;
    fetch(
      window.location.origin +
        `/recommendations/products?product_id=${productId}&limit=4&section_id=cart-related-products&intent=related`
    )
      ...
      .then((text) => {
        ...
        relatedProductsSelect.forEach((element) => {
          element.addEventListener("change", () => {
            const selectedVariant = element.querySelector(
              "option:checked"
            ) as HTMLOptionElement;
            const selectedVariantId = selectedVariant.value;
            const selectedVariantPrice = selectedVariant.dataset.price;
            const relatedProduct = element.closest(
              ".cart_related_product__container"
            ) as HTMLElement;
            const relatedProductAdd = relatedProduct.querySelector(
              ".cart_related_product_add"
            ) as HTMLElement;
            const relatedProductPrice = relatedProduct.querySelector(
        ...
        this.relatedProductsAdds = this.querySelectorAll(
          ".cart_related_product_add"
        );
  ...
  private setupEventListeners() {
    ...
    this.itemQuantityDiv?.forEach((div) => {
      ...
      minusBtn?.addEventListener("click", async () => {
        ...
        if (quantity === 1)
          return this.removeProduct(div.getAttribute("data-id"));
        if (quantity > 1) {
          this.loaderProductCart?.setAttribute("active", "");

          await this.fetchData.updateCart(
  ...
  private async updateCart(html: string) {
    try {
      ...
      const newCartItems = doc.querySelector(
        "cart-drawer .cart_products"
      )?.innerHTML;
      const newCartSubtotal = doc.querySelector(
        "cart-drawer .cart_subtotal_price"
      )?.innerHTML;
      const newCartItemCount = doc.querySelector(
        "cart-drawer .cart__item_count"
      )?.innerHTML;

      if (newCartItems && this.cartProducts)
        this.cartProducts.innerHTML = newCartItems;
      if (newCartSubtotal && this.cartSubtotal)
        this.cartSubtotal.innerHTML = newCartSubtotal;
      if (newCartItemCount && this.cartItemCount)
        this.cartItemCount.innerHTML = newCartItemCount;
      if (newCartItemCount && this.bagItemCount)
        this.bagItemCount.innerHTML = newCartItemCount
          .replace("(", "")
          .replace(")", "");
  ...
  handlePercentagemDiscount() {
    const cartItemDiscount = this.querySelectorAll(
      ".cart_item_discount"
    ...
    cartItemDiscount.forEach((element: HTMLElement) => {
      ...
      const containerPercentage = element.querySelector(
        ".percentage_discount"
      ) as HTMLElement;

      const percentage = this.calculateMonthlyPercentageDifference(
        oldPrice,
        newPrice,
        planText
      );
  ...
  calculateMonthlyPercentageDifference(
    oldInstallment: string,
    ...
    const percentageDifference =
      ((oldValue - newMonthlyValue) / oldValue) * 100;
  ...
  private updateFreeShippingProgress() {
    ...
    if (subtotal >= this.freeShippingPrice) {
      ...
      this.progressSpan.textContent = `${this.formatPrice(remaining)} ${
        this.translations["missing-free-shipping"]
      }`;
      this.cartShippingPrice.textContent =
        this.translations["to_be_calculated"];
    }
  }
  ...
  private extractPrice(priceString: string | null): number {
    ...
    return (
      parseFloat(priceString.replace(/[^0-9,]/g, "").replace(",", ".")) || 0
    );
  }
  ...
  private loadTranslations(): { [key: string]: string } {
    const translationsScript = document.querySelector(
